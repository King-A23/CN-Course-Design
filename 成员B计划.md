# 许久新（2024211130）成员 B 个人计划

> 当前计划已按最终分工更新，成员 B 为许久新（2024211130）。

## 1. 角色定位

许久新（2024211130）负责“事件循环 + 中继转发 + 状态表 + 缓存 + 测试落地”主线，必须保证程序在真实网络环境下稳定工作，并支撑验收中的并发与异常场景。
...
1. `config`、`platform`、`logger`、`pending_map`、`cache`、`server` 和 `main` 已完成并接入主程序。
2. `server` 已按“本地命中 -> Cache 命中 -> 上游转发”的顺序处理查询，并周期性清理 pending 请求和过期 Cache。
3. `pending_map` 已支持上游 ID 分配、客户端 ID 保存、删除和超时回收，并通过 `test_pending_map` 验证。
4. `cache` 已支持写入、命中、TTL 递减、过期清理和 LRU 替换，并通过 `test_cache` 验证。
5. 当前全量单元测试包括 `test_local_table`、`test_dns_packet`、`test_pending_map`、`test_cache`，均已通过。
